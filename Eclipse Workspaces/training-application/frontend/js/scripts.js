...
function orderPizzaP(){
  ...
  fetch('http://localhost:2000/pizza/pizza/create', {
    ...
    fetch('http://localhost:2000/pizza/request/create/' + pizzaId + "/" + localStorage.getItem("id"), {
    method: 'POST',
    headers: {
      'Accept': 'application/json',
      'Content-Type': 'application/json'
...
function login(){
  ...
  fetch('http://localhost:2000/pizza/user/login/' + informedEmail + '/' + informedPassword, {
    ...
    if (loginResponse == 0) {
      ...
      fetch('http://localhost:2000/pizza/user/get-role/' + loginResponse, {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      }.then(response => response.json()).then((personId) =>{
        localStorage.setItem("role", personId)
    })
    })
